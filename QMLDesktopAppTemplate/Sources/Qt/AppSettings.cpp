...
namespace QmlApp
{
	...
	void AppSettings::loadFromFile(const QString& file_path, QSettings::Format format)
	{
		...
		qDebug() << "Loaded settings from file:" << file_path << ". All loaded keys:" << m_settings.allKeys();
	}
	...
	void AppSettings::saveToFile(const QString& file_path, QSettings::Format format)
	{
		...
		qDebug() << "Saved settings to file:" << file_path << ". All saved keys:" << file_settings.allKeys();
	}

	/**
	 * @brief Copies the settings from the source QSettings object to the destination QSettings object.
	 *
	 * This function recursively copies all the settings, including child groups and keys, from the source
	 * QSettings object to the destination QSettings object.
	 *
