...
namespace QmlApp
{
     ...
     *
     * This method attempts to load the Qt provided translation file and the app-specific translation file 
     * for the specified locale. If both files are loaded successfully, the translators are installed and 
     * the languageChanged signal is emitted. If either of the files fails to load, the method attempts 
     * to load the translations for the default language.
     *
    ...
    Q_INVOKABLE bool Translator::load_translation(const QLocale& locale)
    {
        ...
        qDebug() << "Attempting to load translations for the language" << locale << "from" << m_translations_path;
        ...
        if (result)
        {
            ...
            if (result)
            {
                qDebug() << (result ? "Successfully loaded the translators for locale"
                                    : "Failed to load the application translator for locale") << locale;
